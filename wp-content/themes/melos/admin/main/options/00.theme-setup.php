...
function thinkup_title_select() {
	...
	if ( is_page() ) {
		printf( '<span>' . __( '%s', 'melos' ) . '</span>', get_the_title() );
	} elseif ( is_attachment() ) {
		printf( '<span>' . __( 'Blog Post Image: ', 'melos' ) . '</span>' . '%s', esc_attr( get_the_title( $post->post_parent ) ) );
	} else if ( is_single() ) {
		printf( '<span>' . __( '%s', 'melos' ) . '</span>', get_the_title() );
	} else if ( is_search() ) {
		printf( '<span>' . __( 'Search Results: ', 'melos' ) . '</span>' . '%s', get_search_query() );
	} else if ( is_404() ) {
...
global $thinkup_general_breadcrumbdelimeter;
	...
	if ( ! is_front_page() ) {
		...
		if ( is_single() ) {
			...
			$output .= '<span class="breadcrumbs-cat">' . __( 'Archive Category: ', 'melos' ) . '</span>' . get_category_parents($cat, true, '') ;
		} elseif ( is_tag() ) {
